...
For every writing mission, the final Codex response is a required Closeout Packet v3 contract, not optional prose. The prompt enumerates every schema heading, requires an explicit supported review verdict, requires Files Changed to match the actual verified changes, and requires truthful validation/blocker evidence. The parser tolerates safe heading-level and whitespace differences, plus balanced Markdown bullets, inline-code markers, or quotes around a path. It normalizes those wrappers before exact comparison, while rejecting unmatched or embedded formatting, traversal, wildcards, prose, missing headings, unsupported or contradictory verdicts, and file-list mismatches. Runtime v1 does not perform a second closeout-only worker pass; malformed closeouts fail closed.
