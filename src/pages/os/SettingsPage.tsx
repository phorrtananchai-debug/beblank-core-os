    ...
    <section className="space-y-7">
      <header className="command-hero rounded-[36px] border border-black/[0.05] bg-[#faf9f8] p-6 md:p-9">
        <div className="grid gap-6 xl:grid-cols-[1fr_0.42fr]">
          <div>
            <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--bb-text-muted)]">Connector registry / Sheet bridge foundation</p>
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-[var(--bb-text-soft)]">Apps Script bridge scaffolding, Google Sheet source ownership, Finnhub delayed market data foundation, and reviewable sync queue architecture.</p>
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <main className="space-y-5">
          <div className="panel panel-float">
            <div className="panel-header"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--bb-text-muted)]">Connector definitions</p><h3>Registry</h3></div><span className="pill">{data.connectors.length} connectors</span></div>
            <div className="grid gap-3 lg:grid-cols-2">
              {data.connectors.map((connector) => (
                <article key={connector.id} className="rounded-[24px] border border-black/[0.05] bg-[#faf9f8] p-4">
                  <div className="flex items-start justify-between gap-4"><div><p className="font-semibold">{connector.name}</p><p className="mt-1 text-xs text-[var(--bb-text-muted)]">{connector.category} / {connector.syncMode}</p></div><span className="pill">{connector.health}</span></div>
            ...
            <div className="space-y-4">
              {data.sheetSources.map((source) => (
                <article key={source.id} className="rounded-[26px] border border-black/[0.05] bg-[#faf9f8] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-semibold">{source.name}</p><p className="mt-1 text-xs text-[var(--bb-text-muted)]">{source.ownerModule} / {source.authority} / {source.syncState}</p></div><span className="pill">{source.worksheets.length} worksheets</span></div>
              ...
              {data.syncQueue.map((item) => (
                <article key={item.id} className="rounded-[24px] border border-black/[0.05] bg-[#faf9f8] p-4">
                  <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
                    <div><p className="font-semibold">{item.operation} / {item.module}</p><p className="mt-1 text-xs text-[var(--bb-text-muted)]">{item.connectorId} / {item.sourceId} / {item.status} / retries {item.retryCount}</p><pre className="mt-3 overflow-x-auto rounded-2xl bg-white/85 p-3 text-xs text-[var(--bb-text-soft)]">{item.payloadPreview}</pre><p className="mt-2 text-xs text-[var(--bb-text-muted)]">{item.notes}</p></div>
...
const MiniRow = ({ meta, status, title }: { meta: string; status: string; title: string }) => (
  <div className="rounded-2xl border border-black/[0.05] bg-white/75 p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold">{title}</p><p className="mt-1 text-xs text-[var(--bb-text-muted)]">{meta}</p></div><span className="font-mono text-[10px] font-semibold uppercase text-[var(--bb-amber)]">{status}</span></div></div>
)
