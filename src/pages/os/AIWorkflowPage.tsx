    ...
    <section className="space-y-7">
      <header className="command-hero rounded-[36px] border border-black/[0.05] bg-[#faf9f8] p-6 md:p-9">
        <div className="grid gap-6 xl:grid-cols-[1fr_0.42fr]">
          <div>
            <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--bb-text-muted)]">AI Workflow / operational co-pilot layer</p>
...
const Overview = ({ digests, exports, imports, observations, onApproveDigest, onArchiveObservation }: { digests: AIDigestRecord[]; exports: AIExportRecord[]; imports: AIImportRecord[]; observations: AIObservationRecord[]; onApproveDigest: (item: AIDigestRecord) => void; onArchiveObservation: (item: AIObservationRecord) => void }) => (
  <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_360px]">
    <main className="space-y-5">
      <div className="panel panel-float"><div className="panel-header"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--bb-text-muted)]">Operational digests</p><h3>Quiet daily intelligence</h3></div><span className="pill">{digests.length} digests</span></div><div className="grid gap-3 lg:grid-cols-2">{digests.map((digest) => <DigestCard key={digest.id} digest={digest} onApproveDigest={onApproveDigest} />)}</div></div>
...
const MemoryView = ({ memories, onArchiveMemory, reviews }: { memories: AIMemoryRecord[]; onArchiveMemory: (item: AIMemoryRecord) => void; reviews: Array<{ id: string; title: string; module: string; reviewStatus: string; confidence: number }> }) => (
  <section className="grid gap-5 xl:grid-cols-[1fr_0.9fr]"><div className="panel panel-float"><div className="panel-header"><h3>AI memory scaffold</h3><span className="pill">no vector DB yet</span></div><div className="space-y-3">{memories.map((item) => {
  ...
  })}</div></div><div className="panel"><div className="panel-header"><h3>Review history</h3><span className="pill">audit trail</span></div><div className="space-y-3">{reviews.map((item) => <CompactRow key={item.id} title={item.title} meta={`${item.module} / ${item.confidence}% confidence`} status={item.reviewStatus} />)}</div></div></section>
)

const ExportsView = ({ exports }: { exports: AIExportRecord[] }) => (
  <div className="panel"><div className="panel-header"><h3>Context export history</h3><span className="pill">manual handoff</span></div><div className="space-y-4">{exports.map((item) => {
    const confidenceColor = item.confidence >= 75 ? 'green' : item.confidence >= 50 ? 'amber' : 'red'
...
const ImportsView = ({ imports, onApproveImport, onBriefRefinement, onRejectImport }: { imports: AIImportRecord[]; onApproveImport: (item: AIImportRecord) => void; onBriefRefinement: (item: AIImportRecord) => void; onRejectImport: (item: AIImportRecord) => void }) => (
  <div className="panel"><div className="panel-header"><h3>AI import review</h3><span className="pill">preview before apply</span></div><div className="space-y-4">{imports.map((item) => {
    const confidenceColor = item.confidence >= 75 ? 'green' : item.confidence >= 50 ? 'amber' : 'red'
...
const InfoCard = ({ body, label, value }: { body: string; label: string; value: number | string }) => <div className="rounded-[28px] border border-black/[0.05] bg-white/85 p-5"><p className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--bb-text-muted)]">{label}</p><p className="mt-3 text-3xl font-bold">{value}</p><p className="mt-3 text-sm leading-6 text-[var(--bb-text-soft)]">{body}</p></div>
const Metric = ({ label, value }: { label: string; value: number | string }) => <div className="rounded-2xl border border-black/[0.04] bg-white/75 px-4 py-3"><p className="font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-[var(--bb-text-muted)]">{label}</p><p className="mt-2 text-lg font-bold">{value}</p></div>
