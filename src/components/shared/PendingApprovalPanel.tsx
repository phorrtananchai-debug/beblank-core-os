...
export const PendingApprovalPanel = ({ items, onApprove, onReject }: Props) => {
  return (
    <section className="panel">
      ...
      {items.length === 0 ? (
        <p className="text-sm text-[#6e675d]">No pending actions. Queue a page action to preview approval, rejection, ChangeLog, and Snapshot feedback.</p>
      ) : (
        <div className="space-y-3">
          <p className="rounded-2xl border border-[#e7e2d8] bg-white/80 p-3 text-xs text-[#6e675d]">
            Pending state active. Approve applies through the mock adapter; reject writes an audit row to ChangeLog without changing business data.
          </p>
          {items.map((item) => (
