    ...
    <section className="panel">
      ...
      {latest ? (
        <p className="mb-3 rounded-2xl border border-[#e7e2d8] bg-[#faf8f4] p-3 text-xs text-[#6e675d]">
          Latest feedback: {latest.summary} / {formatDateTime(latest.changedAt)}
        </p>
      ) : null}
        ...
        {items.slice(0, 6).map((item) => (
          <div key={item.id} className="rounded-2xl border border-[#e7e2d8] bg-white px-3 py-2">
            ...
            <p className="text-xs text-[#71695f]">
              {item.module} / {item.actionType} / {formatDateTime(item.changedAt)}
            </p>
          </div>
        ))}
        {items.length === 0 && <p className="text-sm text-[#6e675d]">No write activity yet. Approve or reject a pending action to create audit feedback.</p>}
      </div>
    </section>
  )
}
