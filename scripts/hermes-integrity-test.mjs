...
try {
  ...
  test('runner validation allowlist permits only temporary runtime paths', () => {
    assert.equal(isValidationWriteAllowed('.hermes/integrity-fixture/test.json'), true)
    assert.equal(isValidationWriteAllowed('node_modules/.tmp/tsconfig.app.tsbuildinfo'), true)
    assert.equal(isValidationWriteAllowed('dist/assets/app.js'), true)
    assert.equal(isValidationWriteAllowed('src/index.tsx'), false)
    assert.equal(isValidationWriteAllowed('docs/hermes/guide.md'), false)
    assert.equal(isValidationWriteAllowed('package-lock.json'), false)
  })
